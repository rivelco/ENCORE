...
def plot_svd(figures, answer):
    ...
    if plot_widget:
        ...
        for state_idx in range(num_state):
            ...
            row = state_idx // cols
            col = state_idx % cols
            encore_plots.plot_states_from_svd(
                plot_widget, curent_comp, state_idx, row, col
            )
    ...
    if plot_widget:
        encore_plots.plot_ensembles_timecourse(
            plot_widget, neurons_in_ensembles, xlabel="Cell"
        )
...
def plot_pca(figures, answer):
    ...
    try:
        ...
        raise RuntimeError(
            "Error plotting the PCA. Check the other plots and console for more info."
        )
    ...
    if plot_widget:
        encore_plots.plot_delta_rho(
            plot_widget, rho, delta, cents, predbounds, ens_cols
        )
    ...
    try:
        ...
        if plot_widget:
            encore_plots.plot_core_cells(
                plot_widget, ens_cel_corr, [ens_cel_corr_min, ens_cel_corr_max]
            )
    except:
        raise RuntimeError(
            "Error plotting the correlation of cells vs ensembles. Check the other plots and console for more info."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Error plotting the core cells. Check the other plots and console for more info."
        )
    ...
    if plot_widget:
        encore_plots.plot_ensembles_timecourse(
            plot_widget, np.array(answer["sel_ensmat_out"])
        )

    plot_widget = figures.get("pca_plot_cellsinens")
    if plot_widget:
        encore_plots.plot_ensembles_timecourse(
            plot_widget, np.array(answer["sel_core_cells"]).T
        )
...
def plot_ica(figures, answer):
    ...
    if plot_widget:
        encore_plots.plot_cell_assemblies_activity(
            plot_widget, answer["time_projection"]
        )
    ...
    if plot_widget:
        encore_plots.plot_ensembles_timecourse(
            plot_widget, answer["binary_time_projection"], xlabel="Timepoint"
        )
...
def plot_x2p(figures, answer):
    ...
    if plot_widget:
        encore_plots.preview_dataset(
            plot_widget, dataset, xlabel="Neuron", ylabel="Ensemble", cmap="jet"
        )
    # Onsemble activity
    ...
    if plot_widget:
        encore_plots.preview_dataset(
            plot_widget, dataset, xlabel="Timepoint", ylabel="Ensemble", cmap="jet"
        )
    # Offsemble activity
...
def plot_sgc(figures, answer):
    ...
    if plot_widget:
        encore_plots.plot_ensembles_timecourse(
            plot_widget, ensembles_timecourse, xlabel="Timepoint"
        )
...
def plot_example(figures, answer):
    ...
    if plot_widget:
        encore_plots.preview_dataset(
            plot_widget, raster, xlabel="Timepoint", ylabel="Ensembles"
        )
    ...
    if plot_widget:
        ...
        for idx in range(many_dFFo.shape[0]):
            ...
            plot_widget.axes[idx].plot(
                many_dFFo[idx, :], color=colors[idx], linewidth=widths[idx]
            )
