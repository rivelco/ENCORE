...
def run_svd(
    ...
    try:
        ...
        if logger:
            logger(
                "MATLAB engine not available, this function is not available.", "error"
            )
        else:
    ...
    try:
        ...
        if logger:
            logger(
                f"{log_flag} An error occurred while executing the algorithm. Check console logs for more info.",
                "error",
            )
        answer = None
    end_time = time.time()
    ...
    update_params = {}
    if answer:
        update_params = {"pks": int(answer["pks"]), "scut": answer["scut"]}
    else:
        success = False
...
def run_pca(
    ...
    try:
        ...
        if logger:
            logger(
                f"{log_flag} An error occurred while executing the algorithm. Check the Python console for more info.",
                "error",
            )
        answer = None
    end_time = time.time()
    ...
    if answer != None:
        ...
        ensgui_results["neus_in_ens"] = np.array(answer["sel_core_cells"]).T.astype(
            float
        )
    else:
        success = False
...
def run_ica(
    ...
    try:
        ...
        if logger:
            logger(
                f"{log_flag} An error occurred while executing the algorithm, while looking for patterns. Check the Python console for more info.",
                "error",
            )
        answer = None
    if logger:
    ...
    if answer != None:
        ...
        try:
            ...
            if logger:
                logger(
                    f"{log_flag} An error occurred while executing the algorithm, looking for assembly activity. Check the Python console for more info.",
                    "error",
                )
            answer = None
        if logger:
...
def run_x2p(
    ...
    if answer != None:
        ...
        clean_answer["OnsembleActivity"] = np.array(
            answer["Ensembles"]["OnsembleActivity"]
        )
        clean_answer["OffsembleActivity"] = np.array(
            answer["Ensembles"]["OffsembleActivity"]
        )
        ...
        new_clean["Vectors"] = {}
        for ens_it in range(cant_ens):
            new_clean["Durations"][f"{ens_it}"] = np.array(
                answer["Ensembles"]["Durations"][ens_it]
            )
            new_clean["Indices"][f"{ens_it}"] = np.array(
                answer["Ensembles"]["Indices"][ens_it]
            )
            new_clean["Vectors"][f"{ens_it}"] = np.array(
                answer["Ensembles"]["Vectors"][ens_it]
            )
...
def run_sgc(
    ...
    if pars_validated["use_first_derivative"]:
        dx = np.gradient(
            dFFo, axis=1
        )  # Axis 1 to get the derivative of the signal of every neuron
    ...
    if answer != None:
        ...
        activity_raster_peaks = np.array(
            [
                int(np.array(raster_peak)[0][0]) - 1
                for raster_peak in activity_raster_peaks_raw
            ]
        )
        i_assembly_patterns_raw = answer["assembly_pattern_detection"][
            "assemblyIActivityPatterns"
        ]
        i_assembly_patterns = [
            np.array(list(assembly[0])).astype(int)
            for assembly in i_assembly_patterns_raw
        ]
...
def run_example(
    ...
    neurons_in_ensembles_thresholded = (neurons_in_ensembles > threshold).astype(
        np.int_
    )
    ...
    results = {
        ...
        "update_params": {variable_to_update_name: new_value},
        "algorithm_time": algorithm_time,
        "success": True,
    }
