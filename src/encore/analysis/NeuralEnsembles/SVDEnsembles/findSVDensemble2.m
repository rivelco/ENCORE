function [core_svd,state_pks_full,param] = findSVDensemble2(data,param)
...
%% set parameters
if ~isfield(param,'pks'); param.pks = 3; end
...
% if ~isfield(param,'pks') || isempty(param.pks); param.pks = 3; end
% if ~isfield(param,'ticut')|| isempty(param.ticut); param.ticut = 0.22; end
% if ~isfield(param,'jcut')|| isempty(param.jcut); param.jcut = 0.06; end
% if ~isfield(param,'state_cut')|| isempty(param.state_cut); param.state_cut = round(size(data,1)/4); end

pks = param.pks;
ticut = param.ticut;
jcut = param.jcut;
state_cut = param.state_cut;
if isempty(state_cut)
    state_cut = round(size(data,1)/4);
end
...
[state_raster,state_pks,fac_cut] = SVDStateBinary(single(js>jcut),state_cut);
...
for ii = 1:num_state
    ...
    for n = 1:length(ti_vec)
        core_vec = zeros(size(data_active,1),1,'single');
